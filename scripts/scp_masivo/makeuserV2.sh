	...
	do
			read -r -p "Indique el Nivel de Permisologia que estableceras al grupo $grupo siendo: 1) FULL, 2) MEDIO, 3) BAJO: " permissionlevel
				...
				1)
					...
					echo "%$grupo       ALL=(ALL)       NOPASSWD:ALL" >> /etc/sudoers.d/full
					...
					echo "***********************************************"
						...
						do
							read -r -p "Indique el nivel de permisos que desea para el grupo $grupo siendo 1) DESARROLLO, 2) TELEFONIA, 3) BASE DE DATOS: " roles
								...
								;;
								2)
									echo "Cmnd_Alias TELF = /etc/init.d/mysqld*, /bin/sh, /usr/bin/tail*, /var/log/messages*, /var/log/cron*, /bin/nano, /bin/vi, /usr/bin/aptitude*, /usr/bin/apt-get*, /usr/bin/make*, /bin/chown*, /bin/chmod*, /etc/init.d/asterisk*, /etc/init.d/snmpd*, /bin/cp*, /bin/mv*, /bin/ln*, /bin/rm*, /usr/sbin/update-alternatives*, /bin/mkdir*" >> /etc/sudoers.d/medio
